...
public interface PlanRepository extends GenericRepository<Plan, Integer> {
    ...
    @Query("SELECT new com.app.kinlock.presentation.pojo.PlanPojo ( " +
            "p.id, p.name, p.active, p.regional.id, p.insurance.id, p.minimumPremium, p.rate, p.ageLimit, p.discount, p.franchise, p.interest, p.segment.name, p.planType.name, p.broker.id, p.broker.name) " +
            "FROM Plan p ")
